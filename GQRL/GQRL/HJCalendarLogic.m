...
@interface HJCalendarLogic ()
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
    HJCalendarDayModel *selectcalendarDay;
}

@end

@implementation HJCalendarLogic
...
- (NSMutableArray *)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        ...
        HJCalendarDayModel *calendarDay = [HJCalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
...
- (void)changStyle:(HJCalendarDayModel *)calendarDay
{
...
-(void)LunarForSolarYear:(HJCalendarDayModel *)calendarDay{
